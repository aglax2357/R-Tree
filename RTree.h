...
#define MAX_KNN_SEARCH_RESULTS_NUM 8192
...
public:
    ...
    /// k-NN search
    int KNNSearch(const ELEMTYPE a_min[NUMDIMS], bool __cdecl a_resultCallback(DATATYPE a_data, void* a_context), int resultsNums);
...
protected:
    ...
    {
        Rect m_rect;                                  ///< Bounds
        ELEMTYPEREAL m_minDist = -1, m_minMaxDist = -1;             ///< Min Distance to P, min max Distance to P
    ...
    struct Node
    {
        ...
        int m_sortedBranchIndex[MAXNODES];
    };
    ...
    struct KNNSearchVars
    {
        int m_count = 0;
        Rect m_p;
        int m_k;
        DATATYPE m_resultsBuf[MAX_KNN_SEARCH_RESULTS_NUM];
        ELEMTYPEREAL m_distsBuf[MAX_KNN_SEARCH_RESULTS_NUM];
        ELEMTYPEREAL m_min = INFINITY;
    };
    ...
    ELEMTYPEREAL MinDistance(Rect* p, Rect* a_rect);
    ELEMTYPEREAL MinMaxDistance(Rect* p, Rect* a_rect);
    void kNNAtomSearch(KNNSearchVars& var, Node *a_node, int resultsNums);
    void InitKNNSearchVars(KNNSearchVars &a_kVars);
    void SortMinDists(KNNSearchVars &a_Vars, int resultsNums);
    void QSortMinDists(KNNSearchVars &a_Vars, int high, int low);
...
RTREE_TEMPLATE
void RTREE_QUAL::kNNAtomSearch(KNNSearchVars& var, Node *a_node, int resultsNums)
{
    for(int index = 0; index < a_node->m_count; index++)
    {
        a_node->m_branch[index].m_minDist = MinDistance(&(var.m_p), &a_node->m_branch[index].m_rect);
        a_node->m_branch[index].m_minMaxDist = MinMaxDistance(&(var.m_p), &a_node->m_branch[index].m_rect);
        /*printf("RECT %f, %f, %f, %f, %f, %f, min %f minmax %f \n",
               a_node->m_branch[index].m_rect.m_min[0],
               a_node->m_branch[index].m_rect.m_min[1],
               a_node->m_branch[index].m_rect.m_min[2],
               a_node->m_branch[index].m_rect.m_max[0],
               a_node->m_branch[index].m_rect.m_max[1],
               a_node->m_branch[index].m_rect.m_max[2],
               a_node->m_branch[index].m_minDist,
               a_node->m_branch[index].m_minMaxDist);*/
        
    }
    for(int i = 0; i < a_node->m_count; i++)
    {
        if(i == 0)
            a_node->m_sortedBranchIndex[i] = i;
        else
        {
            int j;
            for(j = i - 1; j >= 0; j--)
            {
                if(a_node->m_branch[i].m_minMaxDist < a_node->m_branch[a_node->m_sortedBranchIndex[j]].m_minMaxDist)
                    a_node->m_sortedBranchIndex[j + 1] = a_node->m_sortedBranchIndex[j];
                else
                {
                    break;
                }
            ...
            a_node->m_sortedBranchIndex[j + 1] = i;
        }
    ...
    for(int index = 0; index < a_node->m_count; index++)
    {
        ...
        {
            if(resultsNums >= a_node->m_count || a_node->m_branch[index].m_minDist <= a_node->m_branch[a_node->m_sortedBranchIndex[resultsNums - 1]].m_minDist)
            {
                kNNAtomSearch(var, a_node->m_branch[index].m_child, resultsNums);
            }
            else
            {
                printf("pruned one\n");
        ...
        {
            var.m_distsBuf[var.m_count] = a_node->m_branch[index].m_minDist;
            var.m_resultsBuf[var.m_count] = a_node->m_branch[index].m_data;
            var.m_count++;
        }
    }
...
RTREE_TEMPLATE
int RTREE_QUAL::KNNSearch(const ELEMTYPE *a_min, bool __cdecl a_resultCallback(DATATYPE a_data, void* a_context), int resultsNums)
{
    KNNSearchVars m_kVars;
    InitKNNSearchVars(m_kVars);
    for(int i = 0; i < NUMDIMS; i++)
    {
        m_kVars.m_p.m_min[i] = a_min[i];
    }
    
    kNNAtomSearch(m_kVars, m_root, resultsNums);
    for(int i = 0; i < m_kVars.m_count; i++)
    {
        printf("kNN %d, %f\n", m_kVars.m_resultsBuf[i], m_kVars.m_distsBuf[i]);
    }
    printf("\n");
    SortMinDists(m_kVars, resultsNums);
    for(int i = 0; i < resultsNums; i++)
    {
        printf("kNN %d, %f\n", m_kVars.m_resultsBuf[i], m_kVars.m_distsBuf[i]);
    }
    return 0;
...
RTREE_TEMPLATE
ELEMTYPEREAL RTREE_QUAL::MinDistance(Rect *p, Rect *a_rect)
{
    ELEMTYPEREAL sum = 0;
    for(int i = 0; i < NUMDIMS; i++)
    {
        if(p->m_min[i] < a_rect->m_min[i])
        {
            sum += pow(a_rect->m_min[i] - p->m_min[i], 2);
        }
        else if(p->m_min[i] > a_rect->m_max[i])
        {
            sum += pow(p->m_min[i] - a_rect->m_max[i], 2);
        }
    }
    return (ELEMTYPEREAL) sum;
}

RTREE_TEMPLATE
ELEMTYPEREAL RTREE_QUAL::MinMaxDistance(Rect *p, Rect *a_rect)
{
    if(a_rect->m_min[0] == 8 && a_rect->m_max[1] == 12.4)
    {
        int w = 1;
        printf("%d", w);
    }

    ELEMTYPEREAL rm[NUMDIMS];
    ELEMTYPEREAL rM[NUMDIMS];
    for(int i = 0; i < NUMDIMS; i++)
    {
        if(p->m_min[i] <= (a_rect->m_min[i] + a_rect->m_max[i]) / 2.0)
        {
            rm[i] = a_rect->m_min[i];
            rM[i] = a_rect->m_max[i];
        }
    ...
    ELEMTYPEREAL tmpSum = 0;
    for(int i = 0; i < NUMDIMS; i++)
    {
        tmpSum += pow(std::abs(p->m_min[i] - rM[i]), 2);
    }
    ELEMTYPEREAL min = INFINITY;
    for(int i = 0; i < NUMDIMS ; i++)
    {
        ELEMTYPEREAL tmp = tmpSum - pow(std::abs(p->m_min[i] - rM[i]), 2) + pow(std::abs(p->m_min[i] - rm[i]), 2);
        if(tmp < min)
            min = tmp;
    }
    return min;
...
RTREE_TEMPLATE
void RTREE_QUAL::InitKNNSearchVars(KNNSearchVars &kVars)
{
    for(int i = 0; i < MAX_KNN_SEARCH_RESULTS_NUM; i++)
    {
        kVars.m_distsBuf[i] = INFINITY;
        kVars.m_resultsBuf[i] = -1;
    }
    kVars.m_count = 0;
}

RTREE_TEMPLATE
void RTREE_QUAL::SortMinDists(KNNSearchVars &a_Vars, int resultsNums)
{
    QSortMinDists(a_Vars, a_Vars.m_count - 1, 0);
}

RTREE_TEMPLATE
void RTREE_QUAL::QSortMinDists(KNNSearchVars &a_Vars, int high, int low)
{
    if(low >= high)
    {
        return;
    ...
    int first = low;
    int last = high;
    ELEMTYPEREAL key = a_Vars.m_distsBuf[first];
    int keyIndex = a_Vars.m_resultsBuf[first];/*用字表的第一个记录作为枢轴*/
    
    while(first < last)
    {
        while(first < last && a_Vars.m_distsBuf[last] >= key)
        {
            --last;
        }
        
        a_Vars.m_distsBuf[first] = a_Vars.m_distsBuf[last];/*将比第一个小的移到低端*/
        a_Vars.m_resultsBuf[first] = a_Vars.m_resultsBuf[last];
        
        while(first < last && a_Vars.m_distsBuf[first] <= key)
        {
            ++first;
        }
        
        a_Vars.m_distsBuf[last] = a_Vars.m_distsBuf[first];/*将比第一个小的移到低端*/
        a_Vars.m_resultsBuf[last] = a_Vars.m_resultsBuf[first];

        /*将比第一个大的移到高端*/
    }
    a_Vars.m_distsBuf[first] = key;/*将比第一个小的移到低端*/
    a_Vars.m_resultsBuf[first] = keyIndex;

    QSortMinDists(a_Vars, first - 1, low);
    QSortMinDists(a_Vars, high, first + 1);
}
